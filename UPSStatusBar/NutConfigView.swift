...
struct NutConfigView: View {
    ...
    private func saveConfig() {
        ...
        if let ups = selectedUPS { UserDefaults.standard.setValue(ups, forKey: "NUTSelectedUPS") } 
    ...
    private func testConnectionAndDiscoverUPS() {
        ...
        monitor.discoverUPS(host: trimmedHost, port: port, user: user, password: password) { upsList in
    ...
    private func fetchDetailsForSelectedUPS() {
        ...
        monitor.fetchVarsForUPS(upsName: upsName, host: hostTrim, port: portInt, user: user, password: password) { result in
            DispatchQueue.main.async {
